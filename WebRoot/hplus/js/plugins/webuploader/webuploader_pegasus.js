jQuery(function() {
	...
	if(!$("#uploader.wu-example")){
        ...
        //素材分类
        $materialClassifyId=$('#MATERIAL_CLASSIFY');
        // 添加的文件数量
        fileCount = 0,
    ...
    var roleId=$fileMasterid.val();
	  if(roleId!=''){
		  $.ajax({
				type: "POST",
				url: 'materialUploads/listData?tm='+new Date().getTime(),
				...
				success: function(data){
					$.each(data, function(i,n){
						...
						n.size=n.FILE_SIZE;
						n.type=n.FILE_TYPE;
						n.filePaht='materialUploads/download?ID='+n.ID;
						n.rotation=0;
						n.loaded=1;
    ...
    uploader = WebUploader.create({
        ...
        //sendAsBinary:true,
        chunked: true,
        server: 'materialUploads/fileUpd',
        fileNumLimit: 50,
        compressSize:1024,
        compress:{
            width: 800,
        ...
        accept: {
        	  ...
        	  mimeTypes: 'image/jpg,image/jpeg,image/png,image/gif'   //修改位置
        },
        formData:{
        	MATERIAL_CLASSIFY:$materialClassifyId.val(),
            ...
            switch ( index ) {
                case 0:
                	...
                	if(file.state=='complete'){
                		...
                		$.ajax({
        					type: "POST",
        					url: 'materialUploads/delete?tm='+new Date().getTime(),
        			    	data: {IDS:fileId},
        					dataType:'json',
        					cache: false,
        					success: function(data){
                ...
                case 2:
                    ...
                    break;
                case 3://下载
                	window.open("materialUploads/download?ID="+file.fileId);
                	break;
            }
    ...
    $upload.on('click', function() {
    	if($materialClassifyId.val()==""){
    		layer.msg("请先选择素材分类");
    	}else{
    		//素材分类id上传时传入
    		uploader.options.formData.MATERIAL_CLASSIFY=$materialClassifyId.val();
