...
function saveD() {
    ...
    var fields = dname + ',luyun,' + dtype + ',luyun,' + dbz + ',luyun,' + isQian + ',luyun,' + ddefault;
...
function editField(value, msgIndex) {
    var efieldarray = value.split(',luyun,');
...
function appendC(value) {
    ...
    var fieldarray = value.split(',luyun,');
...
function isSame(value) {
    for (var i = 0; i < arField.length; i++) {
        var array0 = arField[i].split(',luyun,')[0];
        if (array0 == value) {
            return false;
        }
    }
