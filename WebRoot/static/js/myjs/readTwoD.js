...
$(document).ready(function () {
    ...
    $("#uploadify1").uploadify({
        ...
        'folder': locat + "/uploadFiles/QRCode",//上传文件存放的路径,请保持与uploadFile.jsp中PATH的值相同
        'queueId': "fileQueue",
        ...
        'buttonText': "files",
        'scriptData': {'uploadPath': '/uploadFiles/QRCode/'},//这个参数用于传递用户自己的参数，此时'method' 必须设置为GET, 后台可以用request.getParameter('name')获取名字的值
        'method': "GET",
...
function createTwoD() {
    ...
    $.ajax({
        type: "POST",
        url: locat + '/tool/createQRCode.do',
        ...
        success: function (data) {
            ...
            if ("success" == data.result) {
                ...
                $("#encoderImgId").attr("src", locat + '/uploadFiles/QRCode/' + data.encoderImgId);
            } else {
                $("#encoderContent").tips({
                    side: 3,
                    msg: '生成失败,后台有误',
                ...
                $("#encoderImgId").attr("src", locat + "/uploadFiles/QRCode/default.png");
                return;
            }
