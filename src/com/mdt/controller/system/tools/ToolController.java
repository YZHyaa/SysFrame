package com.mdt.controller.system.tools;
...
public class ToolController extends BaseController {
	...
	public Object severTest() {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Map<String, String> map = InterfaceTestUtil.testInterface(pd);
...
//	}
	...
	public ModelAndView goQRCode() throws Exception {
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("system/tools/QRCode");
		mv.addObject("pd", pd);
		return mv;
	}
		...
		if (null == encoderContent) {
			...
			try {
				String filePath = PathUtil.getClasspath()
						+ Const.FILEPATHQRCode + encoderImgId; // 存放路径
				QRCode.encoderQRCode(encoderContent, filePath, "png"); // 执行生成二维码
			} catch (Exception e) {
				errInfo = "error";
			}
		}
	...
	public Object readQRCode() {
		...
		if (null == imgId) {
			...
			try {
				String filePath = PathUtil.getClasspath()
						+ Const.FILEPATHQRCode + imgId; // 存放路径
				readContent = QRCode.decoderQRCode(filePath);// 执行读取二维码
			} catch (Exception e) {
				errInfo = "error";
			}
		}
