package com.mdt.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("PKSluyun")){
			this.userjoin(message.replaceFirst("PKSluyun", ""),conn);
		}if(null != message && message.startsWith("LeavePKSluyun")){
			this.userLeave(conn);
		}if(null != message && message.contains("luyunadmin886")){
			String toUser = message.substring(message.indexOf("luyunadmin886")+10, message.indexOf("luyunluyunadmin888"));
			message = message.substring(0, message.indexOf("luyunadmin886")) +"[私信]  "+ message.substring(message.indexOf("luyunluyunadmin888")+12, message.length());
